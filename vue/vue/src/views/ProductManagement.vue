      ...
      <div v-if="activeIndex==3">
        <el-table
          ...
          <el-table-column
              ...
              width="100">
            <template slot-scope="scope">
              <el-button style="margin-left: 10%" @click="analysisone(scope.row)" type="text" size="small">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <div>
      ...
      >
        <el-form ref="form" :model="producteditOne" label-width="80px">
          ...
          <el-form-item v-if="(dialogVisible3===true||activeIndex==2)&&marketrate.length>0" label="市场占比">
            <el-divider  ><el-button @click="toezro" type="text" >归零 </el-button></el-divider>
              <div v-for="(item , index) in marketrate" style="margin-bottom: 1%">
                <span >{{item.name+'占比：'}}</span>
                <el-input-number  v-model="marketrate[index].rate" :precision="2" :step="0.01" step-strictly="true" ></el-input-number>
              </div>
            <el-divider ><el-button @click="makeezro" type="text" >标准化</el-button></el-divider>
          </el-form-item>
          ...
          <el-form-item v-if="dialogVisible3===true" label="利率%">
      ...
      >
        <div style="padding: 2%">
          <el-tabs v-model="activeNameanalysis" >
            <el-tab-pane label="产品分类" :name="1">
              <el-tabs v-model="coosecalss" type="border-card"  @tab-click="handleClick" >
                <el-tab-pane label="产品分类" :name="1" >
                  <div style="padding: 2%" >
                    ...
                    <div style="display: flex;flex-wrap:wrap;margin-top: 2%">
                      <el-card v-for="(item ,index) in productcalsses" style="margin-right: 2%;margin-bottom:2%;width: 30%" >
                        <div style="display: flex;justify-content: space-between">
                          <h3>{{'分类:'+item.name}}</h3>
                          <div style="display: flex">
                            <el-button style="margin-right: 1%" size="medium" type="text" @click="includeproduct(item)"> 包含产品 </el-button>
                            <el-popconfirm
                                title="确定删除该分类吗？"
                                @confirm="deletcalss(item)"
                            >
                              <el-button slot="reference" style="color: red" size="medium" type="text"> 删除 </el-button>
                            </el-popconfirm>
                          </div>
                        </div>
                        <el-divider></el-divider>
                ...
                <el-tab-pane label="分类分析" :name="3"  >
                  <div v-show="coosecalss==3" style="padding: 2%" >
                    <div style="display: flex">
...
export default {
  ...
  data(){
    return {
      ...
      producteditOne:{id:null,name:'',type:'',rate:'',minday:'',createtime:'',risk:'',description:'',amount:'',price:'',sum:'',targetmarket:[],historicalrate:[],marketrate:[]},
      rates:[],
      targetmarket:[],
      choosenproducts:[],
      analysisproductone:{},
      ...
      namelists:[],
      innerVisible:false,
      productclassanalysis:[],
      psumdata:[],
      csumdata:[['score', '总金额', '产品名称']],
      jmin:0,
      jmax:0,
      marketrate:[]
    }
  ...
  methods:{
    ...
    editproduct(val){//产品编辑
      ...
      let rates=JSON.parse(this.producteditOne.marketrate)
      this.marketrate=[]
      for(let i=0;i<this.targetmarket.length;i++){
        let v={name:'',id:0,rate:0}
        for(let j=0;j<this.marketname.length;j++){
          if(this.targetmarket[i]==this.marketname[j].id){
            v.id=this.targetmarket[i]
            v.name=this.marketname[j].marketname
            v.rate=rates[i]
          }
        }
        this.marketrate.push(v)
      }
    ...
    editSure(){//确定产品编辑或新增
      if(this.dialogVisible3==false){//编辑
        if(this.activeIndex==2){//下架的编辑
          ...
          this.makeezro()
          let marketrates=[];
          for (let i=0;i<this.marketrate.length;i++){
            marketrates.push(this.marketrate[i].rate)
          }
          this.producteditOne.marketrate=JSON.stringify(marketrates)
        }
        let msg=this.test()
        if(msg!=''){
          this.$message({
    ...
    test(){//输入验证
      ...
      for(let i=0;i<this.marketrate.length;i++){
        if(this.marketrate[i].rate>=0){
          sum=sum+Number(this.marketrate[i].rate)
        }
      }
      if (msg!=''){
        msg=msg+'字段不能为空;'
    ...
    selectmarket(val){//新增产品时选择的目标市场
      ...
      for(let i=0;i<val.length;i++){
        let vuale={name:'',id:'',rate:0}
        for(let j=0;j<this.marketname.length;j++){
          if(val[i]==this.marketname[j].id){
            vuale.name=this.marketname[j].marketname
            vuale.id=val[i]
            break
          }
        }
        this.marketrate.push(vuale)
      }
    },
    ...
    handleClick(e,a){//分类分析切换画图
      console.log("切换了")
      if(this.coosecalss==3){
        this.$nextTick(()=>{
          this.analysisclass1(this.psumdata)
          this.analysisclass2(this.csumdata,this.jmin,this.jmax)
        })

      }
    },
    showclasses(){//分类分析（展示哪个类别买的最多，哪个类别人数最多）
      request.post("/productanalysis/getclassesanalysis",{}).then(res => {
        if(res.code==='200'){
          this.productanaylsisdata=res.data
          this.psumdata=[]
          this.csumdata=[['score', '总金额', '产品名称']]
          let min=Number((this.productanaylsisdata[0].sum/ this.productanaylsisdata[0].buyp).toFixed(1))
          let max=min
          for(let i=0;i<this.productanaylsisdata.length;i++){
            let vuale1={ value: this.productanaylsisdata[i].buyp, name: '分类：'+this.productanaylsisdata[i].productClass.name }
            let vuale2=[]
            vuale2.push(Number((this.productanaylsisdata[i].sum/ this.productanaylsisdata[i].buyp).toFixed(1)))
            if(min>vuale2[0]){
              min=vuale2[0]
            }
            if(max<vuale2[0]){
            ...
            vuale2.push(this.productanaylsisdata[i].sum)
            vuale2.push( '分类：'+this.productanaylsisdata[i].productClass.name )
            this.jmin=min
            this.jmax=max
            this.psumdata.push(vuale1)
            this.csumdata.push(vuale2)
          }
        }
      })
    },
    analysisclass1(data){//分类购买人数图
      let myChart = echarts.getInstanceByDom(document.getElementById("productall5"))
      // 如果不存在，就进行初始化
      if (myChart == null) {
        myChart = echarts.init(document.getElementById("productall5"));
      }
      let option;
      const piePatternSrc =
    ...
    analysisclass2(data,min,max){//分类购买金额图
      console.log(data)
      let myChart = echarts.getInstanceByDom(document.getElementById("productall6"))
      // 如果不存在，就进行初始化
      if (myChart == null) {
        myChart = echarts.init(document.getElementById("productall6"));
      }
      let option;
      option = {
        dataset: {
    ...
    deletcalss(e){//删除该分类
      console.log(e)
      request.post("/productanalysis/deleteclass",{
        id:e.id
      }).then(res => {
        if(res.code==='200'){
          this.getclasses()
    ...
    toezro(){//新增市场占比归零
      for(let i=0;i<this.marketrate.length;i++){
        this.marketrate[i].rate=0
      }
    },
    makeezro(){//新增市场占比标准化
      let sum=0
      for(let i=0;i<this.marketrate.length;i++){
        sum=sum+this.marketrate[i].rate
      }
      for(let i=0;i<this.marketrate.length;i++){
        this.marketrate[i].rate=Number((this.marketrate[i].rate/sum).toFixed(2))
      }
    }
  }
}
