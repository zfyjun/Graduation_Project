				...
				<view v-for="(item,index) in userProductsnow" style="margin-top: 3%;">
					<view class="box"  >
						<view style="padding: 3%;">
							...
							<u--text size="16" v-if="(item.balance-item.cost)>=0" type="success" :text="'当前收益：￥'+(item.balance-item.cost).toFixed(2)"></u--text>
							<u--text size="16" v-if="(item.balance-item.cost)<0" type="error" :text="'当前亏损：￥'+(item.balance-item.cost).toFixed(2)"></u--text>
							<view style="display: flex;">
