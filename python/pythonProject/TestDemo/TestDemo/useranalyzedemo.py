...
def get_data():
    ...
    url ="D:/课程作业（所有）/毕设/git/springboot/python/pythonProject/TestDemo/csv/train_set.csv"
    df = pd.read_csv(url)
...
def predict():
    # 载入数据
    url = "D:/课程作业（所有）/毕设/git/springboot/python/pythonProject/TestDemo/csv/test_set.csv"
    data = pd.read_csv(url)
    X = data[['age', 'balance', 'day', 'duration', 'campaign', 'pdays']]
    y = data['y']
    scaler = StandardScaler()
